  ...
  return (
    <div className="flex h-[calc(100vh-4rem)] lg:h-screen bg-gray-50 max-w-[1600px] mx-auto border-x border-gray-200 overflow-hidden animate-in fade-in zoom-in-95 duration-500">
          ...
          {sidebarOpen && (
            <>
              ...
              <motion.aside initial={{ x: -280 }} animate={{ x: 0 }} exit={{ x: -280 }} transition={{ type: 'spring', damping: 25, stiffness: 250 }}
                className="fixed left-0 top-0 bottom-0 w-64 sm:w-72 z-40 bg-white border-r border-gray-200 flex flex-col lg:hidden">
                ...
                <div className="px-3 py-2 border-b border-gray-200 shrink-0 flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-[10px] text-gray-400 mt-0.5">{completedTasks.length}/{curriculum.reduce((sum, m) => sum + m.topics.length, 0)}</p>
                  </div>
      ...
      <div className="flex-1 flex flex-col h-full bg-gray-50 relative">
        <button onClick={() => setSidebarOpen(!sidebarOpen)} className="absolute top-3 left-3 z-20 bg-white border border-gray-200 text-gray-500 p-1.5 sm:p-2 rounded-lg hover:bg-gray-100 shadow-sm"><Menu className="w-4 h-4 sm:w-5 sm:h-5" /></button>

        <div ref={contentRef} className="flex-1 overflow-y-auto px-3 py-6 sm:px-8 md:px-16 sm:py-12 custom-scrollbar">
          ...
          {activeTopic ? (
            <motion.div key={activeTopic.id} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }} transition={{ duration: 0.3 }} className="max-w-3xl mx-auto">
              ...
              <div className="mb-4 sm:mb-6 space-y-2 sm:space-y-3">
                <div className="flex items-center gap-2 text-xs font-semibold text-accent tracking-wider uppercase">
                  <span className="truncate max-w-[150px] sm:max-w-[200px] md:max-w-none">{activeModule?.title}</span>
                  <span className="text-gray-300">/</span>
                  <span className="text-gray-400 font-mono shrink-0">T{topicIndex + 1}</span>
                </div>
              ...
              {/* Navigation */}
              <div className="mt-6 pt-4 border-t border-gray-200 flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-2 pb-8">
                <button onClick={handlePrevTopic} className="flex items-center justify-center gap-1.5 px-3 py-2 sm:px-4 sm:py-3 rounded-lg font-bold transition-all text-gray-500 bg-white border border-gray-200 hover:text-gray-800 hover:border-gray-300 shadow-sm text-sm"><ChevronLeft className="w-4 h-4" /> Previous</button>
                <button onClick={() => { if (isCompleted) toggleTask(activeTopic!.id); else setShowQuiz(true); }}
                  className={`flex items-center justify-center gap-1.5 px-4 py-2 sm:px-5 sm:py-3 rounded-lg font-bold transition-all shadow-sm text-sm ${
                  ...
                  {isCompleted ? <><CheckCircle2 className="w-4 h-4" /> Done</> : 'Quiz'}
                </button>
                {hasNextTopic && <button onClick={handleNextTopic} className="flex items-center justify-center gap-1.5 px-3 py-2 sm:px-4 sm:py-3 rounded-lg font-bold transition-all text-gray-500 bg-white border border-gray-200 hover:text-gray-800 hover:border-gray-300 shadow-sm text-sm">Next <ChevronRight className="w-4 h-4" /></button>}
              </div>
