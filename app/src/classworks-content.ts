...
(3 rows)`,
  ...
  "w12-d3": [
    {
      task: `Write an INSERT INTO ... WHERE NOT EXISTS query that inserts a new employee named 'Eve' with salary 90000 into the employees table, but only if no employee with that name already exists.`,
      language: "sql",
      code: `INSERT INTO employees (name, salary, department_id)\nSELECT 'Eve', 90000, 1\nWHERE NOT EXISTS (\n  SELECT 1 FROM employees WHERE name = 'Eve'\n);`,
      expectedOutput: ``,
      hint: `Use INSERT INTO ... SELECT ... WHERE NOT EXISTS (SELECT 1 FROM ... WHERE ...).`,
    },
  ],
  "w12-d3-b": [
    {
  ...
  "w12-d4": [
    {
      task: `What does EXPLAIN ANALYZE do in SQL, and what would you look for in the output to diagnose a slow query on a large table?`,
      language: "text",
      code: ``,
      expectedOutput: `explain||seq scan||index scan||missing index||execution plan||analyze`,
      hint: `EXPLAIN ANALYZE shows the query plan. Look for Seq Scan (bad on large tables) vs Index Scan (fast).`,
    },
  ],
  "w12-d4-b": [
    {
...
print(df.corr())`,
  ...
  "w16-d4": [
    {
      task: `Write a Python function called validate_orders(df) that checks: (1) required columns 'customer_id' and 'amount' exist, (2) no null values in 'customer_id', and (3) no negative values in 'amount'. Print a warning for each issue found and return False if any issues exist, True otherwise.`,
      language: "python",
      code: `import pandas as pd
...
def validate_orders(df):
    ...
    if 'customer_id' in df.columns and df['customer_id'].isnull().any():
        issues.append('customer_id has null values')
...
df = pd.DataFrame({'customer_id': [1, 2, None], 'amount': [100, -50, 200]})
validate_orders(df)`,
      expectedOutput: ``,
      hint: `Check columns exist first, then check nulls and negatives. Append issues to a list and print them all at the end.`,
    },
  ],
  "w16-d5": [
    {
...
print(f't-statistic: {t_stat:.3f}, p-value: {p_value:.3f}')`,
  ...
  "w24-d2": [
    {
      task: `Business case question: Revenue dropped 15% last month. Write 4 clarifying questions you would ask BEFORE writing any SQL, then write one SQL query that would help you start isolating where the drop came from (segment by product category and month).`,
      language: "sql",
      code: `-- Clarifying questions to ask first:\n-- 1. Is the drop across all product categories or just one?\n-- 2. Is it all customer segments or specific ones (SMB vs Enterprise)?\n-- 3. Is it all regions or one geography?\n-- 4. Is this seasonal - same month last year?\n\n-- Segmentation query to isolate the source:\nSELECT product, SUBSTR(order_date,1,7) AS month, SUM(amount) AS revenue\nFROM orders\nWHERE order_date >= '2024-09-01'\nGROUP BY product, month\nORDER BY month DESC, revenue ASC;`,
      expectedOutput: ``,
      hint: `Start with clarifying questions to narrow scope. Then segment by product and time period to find where the drop is concentrated.`,
    },
  ],
  "w24-d3": [
    {
