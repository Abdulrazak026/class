        ...
        {tabs.map(tab => (
          <button key={tab.id} onClick={() => setActiveTab(tab.id)}
            ...
            <div className="relative">
              {tab.icon}
              {tab.id === 'studyroom' && unreadCount && unreadCount > 0 ? (
                <span className="absolute -top-1.5 -right-1.5 bg-red-500 text-white text-[9px] font-bold rounded-full min-w-[14px] h-3.5 flex items-center justify-center px-0.5 shadow-sm">
                  {unreadCount > 99 ? '99+' : unreadCount}
                </span>
              ) : null}
            </div>
            {!collapsed && <span>{tab.label}</span>}
            {!collapsed && tab.id === 'studyroom' && unreadCount && unreadCount > 0 ? (
              <span className="ml-auto bg-red-500 text-white text-[9px] font-bold rounded-full px-1.5 py-0.5 shadow-sm min-w-[18px] text-center">
                {unreadCount > 99 ? '99+' : unreadCount}
              </span>
            ) : null}
          </button>
